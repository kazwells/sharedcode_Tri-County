...
#standardized at 1-31-2000

zhvi_one <- zhvi_tri %>%
  group_by(RegionName) %>%
  mutate(
    base_value = ZHVI[Date == ymd("2000-01-31")],
    ZHVI_std = ZHVI / base_value
  ) %>%
  ungroup()
...
#graph
graph_one <- ggplot(zhvi_one, aes(x = Date, y = ZHVI_std, color = RegionName, group = RegionName)) +
  geom_line(size = 1.2) +
  labs(
    title = "Zillow Home Value Index (All Homes)",
    subtitle = "Standardized at 01/31/2000 = 1",
    x = "Date", y = "Home Value Index (USD)",
    color = "County"
  ) +
  scale_color_manual(values = c("San Luis Obispo County" = "maroon", 
...
graph_one
#save as interactive graph
graph_standardized <- ggplotly(graph_one)
saveWidget(graph_raw, "zhvi_one.html", selfcontained = TRUE)
